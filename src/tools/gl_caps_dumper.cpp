...
int main(int argc, char** args) {
    if (argc != 2) {
        std::cerr << "must be one arg" << std::endl;
        return 1;
    }

    if (!ve001::window.init("gl-caps-dumper", 640, 480, nullptr)) {
        return 1;
    }
    ve001::glInit();
    
    std::unordered_map<std::string, GLenum> str_to_enum_map{
    ...
    GLint num[256] = { 0 };
    glGetIntegerv(str_to_enum_map[key], num);
