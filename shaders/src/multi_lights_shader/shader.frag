...
#define SIZEOF_DIRECTIONAL_LIGHT 4
    ...
    for(uint i = 0; i < point_lights_size; ++i) {
        ...
        if (light.shadow_map_index != -1.0) {
            ...
            for (uint face = 0; face < 6; ++face) {
                mat4 light_transform = light.transform[face];
...
vec3 spotLightsContrib(vec3 frag_pos, vec3 view_dir, vec3 normal, vec3 material_diffuse, vec4 material_specular) {
    ...
    for(uint i = 0; i < spot_lights_size; ++i) {
        ...
        result += intensity * attenuation * (ambient + (diffuse + specular));
    }
...
void main() {
    ...
    vec3 result = 
        directionalLightsContrib(in_position, camera_dir, in_normal, tex.xyz * params.diffuse, params.specular) +
        pointLightsContrib(in_position, camera_dir, in_normal, tex.xyz * params.diffuse, params.specular) +
        spotLightsContrib(in_position, camera_dir, in_normal, tex.xyz * params.diffuse, params.specular);
